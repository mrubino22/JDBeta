...
class ParallelotopeRationalDomainFast private (favorAxis: Int) extends NumericalDomain {
  ...
  final class Property(
   ...
   //require(low.length == A.cols)
     ...
     * @throws $ILLEGAL
     */
    def wideningOrig(that: Property): Property = {
     ...
     if (thisRotated < that) {
      ...
      /**
     * @ TEST WIDENING WITH UNION P1 union P2
     * @inheritdoc
     * @note @inheritdoc
     * @throws $ILLEGAL
     */
    def widening(that: Property): Property = {
      ...
      if (isEmpty)
        ...
        val thisRotated = this.rotate(that.A)
        // println("thisR "+thisRotated)
        //  println("That "+that)
         //val PbRotated = that.rotate(Pc.A)
        //if(that.tryCompareTo(thisRotated).equals(None)){
      //   println("thisR "+thisRotated)
      //   println("That "+that)
      //    println("Unione"+ (this unionWidening that))
        //}
   
   // println((that >thisRotated) +" " +that.tryCompareTo(thisRotated).equals(None))
        if(that <= thisRotated || that.tryCompareTo(thisRotated).equals(None)) {
        
      //    if(that <= thisRotated ) {
         
          val Pc=this union that
         // val Pc=that union this
          val PaRotated = this.rotate(Pc.A)
          val newlow = this.low.copy
          val newhigh = this.high.copy
          for (i <- 0 to dimension - 1) {
            if (PaRotated.low(i) > Pc.low(i)) newlow(i) = RationalExt.NegativeInfinity
            if (PaRotated.high(i) < Pc.high(i)) newhigh(i) = RationalExt.PositiveInfinity
          }

          new Property(false, newlow, Pc.A, newhigh)
     }else{
       if (thisRotated < that) {
          val newlow = thisRotated.low.copy
          val newhigh = thisRotated.high.copy
